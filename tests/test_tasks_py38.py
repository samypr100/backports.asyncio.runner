...
# Copied from https://github.com/python/cpython/blob/3.8/Lib/test/test_asyncio/test_tasks.py
# Modified slightly to use from backports.asyncio.runner.tasks import Task as PatchedTask
...
@unittest.skipUnless(
    sys.version_info[:2] == (3, 8), "Skipping as Python Version is not 3.8"
)
class BaseTaskTests:
    Task = None
    Future = None
...
class BaseTaskIntrospectionTests:
    ...
    def test__register_task_3(self):
        ...
        self.assertEqual(asyncio.all_tasks(loop), set())
        with self.assertWarns(DeprecationWarning):
            self.assertEqual(PatchedTask.all_tasks(loop), {task})
        self._unregister_task(task)
...
class PyCurrentLoopTests(BaseCurrentLoopTests, test_utils.TestCase):
    def new_task(self, coro):
        return PatchedTask(coro, loop=self.loop)
...
class GenericTaskTests(test_utils.TestCase):
    def test_future_subclass(self):
        self.assertTrue(issubclass(PatchedTask, tasks._PyTask))
        self.assertTrue(issubclass(PatchedTask, futures._PyFuture))
